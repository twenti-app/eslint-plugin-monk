...
function printSortedItems(sortedItems, originalItems, sourceCode) {
  ...
  const sorted = sortedItems
    .map(groups => groups.map(groupItems => groupItems.map(item => item.code).join(newline)).join(newline))
...
function getImportExportItems(passedChunk, sourceCode, isSideEffectImport, getSpecifiers) {
  ...
  return chunk.map((node, nodeIndex) => {
    ...
    const commentsBefore = sourceCode
      ...
      .filter(
        comment =>
          comment.loc.start.line <= node.loc.start.line && comment.loc.end.line > lastLine && (nodeIndex > 0 || comment.loc.start.line > lastLine)
      );
    ...
    const indentation = getIndentation(commentsBefore.length > 0 ? commentsBefore[0] : node, sourceCode);
    ...
    const trailingSpaces = getTrailingSpaces(commentsAfter.length > 0 ? commentsAfter[commentsAfter.length - 1] : node, sourceCode);
...
function printWithSortedSpecifiers(node, sourceCode, getSpecifiers) {
  ...
  const sorted = flatMap(sortedItems, (item, index) => {
    ...
    const maybeNewline =
      previous != null && needsStartingNewline(item.before) && !(previous.after.length > 0 && isNewline(previous.after[previous.after.length - 1]))
        ? [{ type: 'Newline', code: newline }]
        : [];

    if (index < lastIndex || hasTrailingComma) {
      return [...maybeNewline, ...item.before, ...item.specifier, { type: 'Comma', code: ',' }, ...item.after];
    }
...
function parseWhitespace(whitespace) {
  ...
  return (
    items
      .map((spacesOrNewline, index) => (index % 2 === 0 ? { type: 'Spaces', code: spacesOrNewline } : { type: 'Newline', code: spacesOrNewline }))
  ...
  return flatMap(tokens, (token, tokenIndex) => {
    ...
    return [
      ...
      ...flatMap(comments, (comment, commentIndex) => {
        ...
        return [...parseWhitespace(sourceCode.text.slice(previous.range[1], comment.range[0])), { ...comment, code: sourceCode.getText(comment) }];
      }),
